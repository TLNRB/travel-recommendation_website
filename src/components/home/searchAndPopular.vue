  ...
  <div class="container-fluid bg-gray-100 mx-auto px-4 sm:px-16 lg:px-64 xl:px-80 2xl:px-96 py-8">
    ...
    <div class="mb-8 w-full">
      <div class="flex gap-4 justify-start mb-4 flex-wrap">
        <span v-for="cat in ['All', 'Museum', 'Monument', 'Beach', 'Street', 'Park']" :key="cat"
          @click="selectCategory(cat)" :class="[
      ...
      <div class="relative w-full">
        <input type="text" v-model="searchQuery" @input="showSuggestions = searchQuery.length >= 3"
          placeholder="Search Places..." class="w-full px-4 py-3 border rounded-2xl shadow-md" />

        <!-- Suggestions Dropdown -->
        <ul v-if="showSuggestions && filteredPlaces.length > 0"
          class="absolute z-50 w-full bg-white mt-2 rounded-lg shadow-lg max-h-60 overflow-auto">
          <li v-for="place in filteredPlaces" :key="place.name" @click="goToPlace(place.name)"
            class="px-4 py-2 hover:bg-green-100 cursor-pointer">
            {{ place.name }}
          </li>
        </ul>

        <div v-else-if="showSuggestions && filteredPlaces.length === 0"
    ...
    <div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div v-for="place in mostRatedPlaces" :key="place.name"
