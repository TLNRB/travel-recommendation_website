         ...
         <form @submit.prevent="submit">
            ...
            <div>
               ...
               <div class="flex flex-wrap gap-3 mb-2">
                  <div v-for="(img, index) in editCityImage.images" :key="index"
               ...
               <div v-if="editCityImage.newImages!.length > 0" class="flex flex-wrap gap-2 mt-2">
                  <div v-for="(image, index) in editCityImage.newImages" :key="index"
               ...
               <!-- Image Error -->
               <div v-if="imageError || editCityImage.newImages!.length === 0" class="mt-2 text-red-500 text-sm italic">
                  {{ imageError }}
               </div>
            </div>
...
const handleImageUpload = (event: Event) => {
   ...
   const existingNames = new Set(editCityImage.value.newImages?.map((image: File) => image.name))
   ...
   editCityImage.value.newImages = [...editCityImage.value.newImages!, ...newFiles]
}


const removeImage = (index: number) => {
   editCityImage.value.images.splice(index, 1)
}

const removeNewImage = (index: number) => {
   editCityImage.value.newImages!.splice(index, 1)
}
...
const submit = () => {
   if ((!editCityImage.value.images || editCityImage.value.images.length === 0) && (!editCityImage.value.newImages || editCityImage.value.newImages.length === 0)) {
